...
{
    Node * selected = this->root;

    while(selected != NULL || selected->data != key)
    {
        if(key > selected->data)
        {
...
bool AVL::insert(int key)
{
    ...
    //selected is either null or the data is equal to the insert key
    ...
    {
        selected = new Node(key);
        selected->decr = parent;
    }
}
